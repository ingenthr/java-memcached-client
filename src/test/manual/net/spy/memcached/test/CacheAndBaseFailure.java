 ...
 */
public class CacheAndBaseFailure {

    private MemcachedClient mcc = null;
    private MemcachedClient mcb  = null;

    CacheAndBaseFailure(String uri) throws URISyntaxException, ConfigurationException {

	String username = "abase";
	...
	try {
	    ...
	    mcc = new MemcachedClient(addresses, "default", "", "");
	    mcb = new MemcachedClient(addresses, username, username, password);
     ...
     */
    public static void main(String[] args) throws InterruptedException, URISyntaxException, ConfigurationException {
//	if (args.length != 1) {
//	    System.err.println("Usage example:\nQuickAuthLoad user password localhost:11212 10000");
//	    System.exit(1);
//	}
	CacheAndBaseFailure m = new CacheAndBaseFailure("http://10.1.5.121:8091/pools");
	...
	for (int i = 0; i < iterations; i++) {
	    mcc.set("test" + i, 0, "test" + i);
	}

	for (int i = 0; i < iterations; i++) {
	    Object res = mcc.get("test" + i);
	...
	mcc.shutdown(1, TimeUnit.SECONDS);
	} catch (Exception ex) {
